## Base (1/4)

### Basic Research:
* **Tier**: 1
* **Input**: Pay Mana
* **Output**: Gain T2 Spells
* **Flavor**: Spellwork 101
* **Power Boost**: 0

### Unsafe Lab Conditions:
* **Tier**: 2
* **Input**: Pay Life
* **Output**: Gain T4 Spells
* **Flavor**: "What matters if I cut corners? See what I have learned?"
* **Power Boost**: 0

### Composite Spells:
* **Tier**: 2
* **Input**: Pay T2 Spells
* **Output**: Gain T3 Spells
* **Flavor**: "A little of this, a little of that..."
* **Power Boost**: 0

### Fancy Tools:
* **Tier**: 2
* **Input**: Pay Mana
* **Output**: Gain T3 Spells
* **Flavor**: Coincidentally most of the great mages came from wealthy families
* **Power Boost**: 0

### Team Effort:
* **Tier**: 2
* **Input**: Grant T2 Spells
* **Output**: Gain T3 Spells
* **Flavor**: Disagreements put aside for the moment in search of greatness
* **Power Boost**: 1

### Gather Ingredients:
* **Tier**: 3
* **Input**: None
* **Output**: Gain Mana
* **Flavor**: "Why do we have to waste so much time on this again?"
* **Power Boost**: 0

### Lab Distraction:
* **Tier**: 3
* **Input**: Pay Mana
* **Output**: Drain T4 Spells
* **Flavor**: One swift table flip can have long lasting effects
* **Power Boost**: 0

### Advantages Trade:
* **Tier**: 3
* **Input**: Give T3 Spells
* **Output**: Take T3 Spells
* **Flavor**: You've just got to hope you don't get the same spell back
* **Power Boost**: 0

### Wasteful Channeling:
* **Tier**: 3
* **Input**: Pay Mana
* **Output**: Gain T5 Spells
* **Flavor**: Even the worst mages can do wonderful things with enough raw power
* **Power Boost**: 0

### Reframe the Results:
* **Tier**: 3
* **Input**: Pay T4 Spells
* **Output**: Gain T4 Spells
* **Flavor**: "But if you look at it from this angle it's something completely different!"
* **Power Boost**: 0

### Blatant Scam:
* **Tier**: 3
* **Input**: Give T3 Spells
* **Output**: Take Mana
* **Flavor**: "Well it's 25% cheaper then everyone else sells it"
* **Power Boost**: 0

### Party Tricks:
* **Tier**: 4
* **Input**: Pay Mana
* **Output**: Gain T2 Spells
* **Flavor**: With all the new tricks you've learned making such small spells as those should be trivial
* **Power Boost**: 0

### Efficient Channeling:
* **Tier**: 4
* **Input**: Pay Mana
* **Output**: Gain T5 Spells
* **Flavor**: With the latest spellcraft theories many never before dreamed of things can be created
* **Power Boost**: 0

### Mind Wipe:
* **Tier**: 4
* **Input**: Pay T4 Spells
* **Output**: Drain T3 Spells
* **Flavor**: Sure it needs some of your knowledge to work, but that's hardly relevant
* **Power Boost**: 0

### Sell the Test Answers:
* **Tier**: 4
* **Input**: Give T2 Spells
* **Output**: Take Mana
* **Flavor**: "These new mages don't even try to study anymore"
* **Power Boost**: 0

### Knowledge Explosion:
* **Tier**: 5
* **Input**: Pay Mana
* **Output**: Gain T5 Spells
* **Flavor**: "At my level of power reality itself is as if a book whose pages I can read"
* **Power Boost**: 1

## Base (2/4)

### Grandma's Cookies:
* **Tier**: 1
* **Input**: None
* **Output**: Gain Life
* **Flavor**: A heartwarming treat, which is good, as it won't give you much life
* **Power Boost**: 0

### Campfire Stew:
* **Tier**: 2
* **Input**: Grant Life
* **Output**: Gain Life
* **Flavor**: "Airsick lowlanders"
* **Power Boost**: 1

### Cheap Poison:
* **Tier**: 2
* **Input**: Pay Mana
* **Output**: Drain Life
* **Flavor**: It's not very good, but it's very easy to make, too easy
* **Power Boost**: 1

### Make Way for the New:
* **Tier**: 2
* **Input**: Give Life
* **Output**: Gain Life
* **Flavor**: "No fair, you're not splitting it evenly"
* **Power Boost**: 1

### Quick Cash:
* **Tier**: 2
* **Input**: Pay Life
* **Output**: Gain Mana
* **Flavor**: Some "mages" will even buy your very soul
* **Power Boost**: 1

### Generic Healing Potion:
* **Tier**: 3
* **Input**: Pay Mana
* **Output**: Gain Life
* **Flavor**: It's nothing fancy, but neither is healing
* **Power Boost**: 0

### Fireball:
* **Tier**: 3
* **Input**: Pay Mana
* **Output**: Drain Life
* **Flavor**: "It's easier to let natural forces do the damage"
* **Power Boost**: 0

### Vampiric Bite:
* **Tier**: 3
* **Input**: Pay Mana
* **Output**: Take Life
* **Flavor**: "Just avoid any sun for a little while"
* **Power Boost**: 1

### Frame the Innocent:
* **Tier**: 3
* **Input**: Give Life
* **Output**: Take Mana
* **Flavor**: "See? They stabbed me!"
* **Power Boost**: 0

### Bleed Out Together:
* **Tier**: 3
* **Input**: Pay Life
* **Output**: Drain Life
* **Flavor**: "I'm not going down alone!"
* **Power Boost**: 0

### Free the Mind:
* **Tier**: 3
* **Input**: Pay T3 Spells
* **Output**: Gain Life
* **Flavor**: "I feel so much better with less to worry about"
* **Power Boost**: 0

### Assassin:
* **Tier**: 4
* **Input**: Pay Mana
* **Output**: Drain Life
* **Flavor**: "At least 75% effectiveness guaranteed!
* **Power Boost**: 0

### Top of the line Healing:
* **Tier**: 4
* **Input**: Pay Mana
* **Output**: Gain Life
* **Flavor**: "It's amazing what someone like me has access to that the lower mages can't even dream of"
* **Power Boost**: 0

### Deal with the Devil:
* **Tier**: 4
* **Input**: Pay Life
* **Output**: Gain Mana
* **Flavor**: "It's not a bad deal..."
* **Power Boost**: 0

### Enemies United:
* **Tier**: 4
* **Input**: Grant Life
* **Output**: Gain Life
* **Flavor**: "How about we all take a breather"
* **Power Boost**: 0

### Murder:
* **Tier**: 5
* **Input**: Pay Mana
* **Output**: Drain Life
* **Flavor**: "If you want it done right you've got to do it yourself"
* **Power Boost**: 0

## Base (3/4)

### Basic Optimizations:
* **Tier**: 1
* **Input**: None
* **Output**: Gain Mana Pay Discount
* **Flavor**: The newer mages always make so many little mistakes, there's always something to be improved
* **Power Boost**: 0

### Mana Channel:
* **Tier**: 2
* **Input**: None
* **Output**: Gain Mana Gain Boost
* **Flavor**: Once the flow of Mana has started it's easier to maintain
* **Power Boost**: 0

### Blood in the Water:
* **Tier**: 2
* **Input**: Pay Life
* **Output**: Gain Mana Gain Boost
* **Flavor**: A little pain to show passion never hurts when drawing Mana from beyond
* **Power Boost**: 0

### Artificial Limitations:
* **Tier**: 2
* **Input**: None
* **Output**: Gain Mana Grant Discount
* **Flavor**: "You hardly need so much Mana, be happy with what you've got"
* **Power Boost**: 0

### Kick a Man While He's Down:
* **Tier**: 2
* **Input**: None 
* **Output**: Gain Mana Drain Boost
* **Flavor**: "With their defenses already down it's easy to take a little more"
* **Power Boost**: 0

### Safety Equipment:
* **Tier**: 3
* **Input**: Pay Mana
* **Output**: Gain Life Pay Discount
* **Flavor**: Although rarely used, the school provides a vast collection of PPE for rent at a small charge
* **Power Boost**: 0

### Join the Fight:
* **Tier**: 3
* **Input**: Pay Life
* **Output**: Gain Life Drain Boost
* **Flavor**: "What was I supposed to do? It was three against one"
* **Power Boost**: 0

### Advanced Optimizations:
* **Tier**: 3
* **Input**: None
* **Output**: Gain Mana Pay Discount
* **Flavor**: "Why pay so much Mana when a little extra work cuts the price so much"
* **Power Boost**: 0

### Poison the Health Potion:
* **Tier**: 3
* **Input**: Pay Life
* **Output**: Gain Life Grant Discount
* **Flavor**: "Why sure it'll heal you right up, no customer has complained yet!"
* **Power Boost**: 0

### Forget the Payment:
* **Tier**: 3
* **Input**: None
* **Output**: Gain Mana Grant Discount
* **Flavor**: "Well you didn't *really* need that Mana anyway"
* **Power Boost**: 0

### Overtax:
* **Tier**: 3
* **Input**: None
* **Output**: Gain Mana Drain Boost
* **Flavor**: "It's still the best price around" 
* **Power Boost**: 0

### Poison the Poison:
* **Tier**: 4
* **Input**: None
* **Output**: Gain Life Drain Boost
* **Flavor**: "Whats better then poison? Poisoned poison!"
* **Power Boost**: 0

### Taint the Mana Supply:
* **Tier**: 4
* **Input**: Give Mana 
* **Output**: Gain Mana Drain Boost
* **Flavor**: "A single drop of bad Mana can spoil the whole batch" --Old proverb
* **Power Boost**: 0

### Premium Health Potion:
* **Tier**: 4
* **Input**: None
* **Output**: Gain Life Gain Boost
* **Flavor**: "This stuff tastes way better then the normal potion too"
* **Power Boost**: 0

### Advanced Safety Equipment:
* **Tier**: 4
* **Input**: Pay Mana
* **Output**: Gain Life Pay Discount
* **Flavor**: "It's way better then anything the school provides"
* **Power Boost**: 0

### Skip the Costs:
* **Tier**: 5
* **Input**: None
* **Output**: Gain Mana Pay Discount
* **Flavor**: "What's it going to do? Not work?"
* **Power Boost**: 0

## Base (4/4)

### Brainstorm:
* **Tier**: 1
* **Input**: Pay Mana
* **Output**: Gain Recycle
* **Flavor**: The joys of a beginning mage's first presentation
* **Power Boost**: 1

### Point out a Mana Burst:
* **Tier**: 2
* **Input**: Grant Mana
* **Output**: Gain Freeze
* **Flavor**: "Wait! Look over there!"
* **Power Boost**: 4

### Forced Invention:
* **Tier**: 2
* **Input**: Grant Add
* **Output**: Drain Mana
* **Flavor**: "Looks like my little taunt is paying off"
* **Power Boost**: 1

### Change of Plans:
* **Tier**: 2
* **Input**: Pay Mana
* **Output**: Gain Delayed Ignite
* **Flavor**: "What do you mean I present next? You said I had until next week!"
* **Power Boost**: 4

### Surprise Show:
* **Tier**: 2
* **Input**: Pay Mana
* **Output**: Gain Instant
* **Flavor**: "Whats the best kind of magic? Unexpected magic!"
* **Power Boost**: 4

### Meddle:
* **Tier**: 3
* **Input**: None 
* **Output**: Gain Add
* **Flavor**: "What do you think you're doing?! That'll never work!"
* **Power Boost**: 0

### Distract:
* **Tier**: 3
* **Input**: Grant Add
* **Output**: Gain Add
* **Flavor**: "While their busy I'm sure they won't mind if I fix up their project"
* **Power Boost**: 0

### Move the Deadline:
* **Tier**: 3
* **Input**: Pay Mana
* **Output**: Gain Delayed Void
* **Flavor**: "Well you should have already had this project done anyway"
* **Power Boost**: 1

### Bribe the Scientists:
* **Tier**: 3
* **Input**: Give Life
* **Output**: Gain Freeze
* **Flavor**: "Most mages regardless of position won't turn down a little 'life'"
* **Power Boost**: 0

### Light the Fuse:
* **Tier**: 3
* **Input**: Pay Mana
* **Output**: Gain Delayed Ignite
* **Flavor**: "Well they were hogging all the space anyway"
* **Power Boost**: 1

### Bribe the Guard:
* **Tier**: 3
* **Input**: Give Mana
* **Output**: Gain Add
* **Flavor**: "These guards wouldn't even have seen us if we didn't pay..."
* **Power Boost**: 0

### Collective Amnesia:
* **Tier**: 4
* **Input**: None
* **Output**: Gain Forget
* **Flavor**: "I wonder what all those weird runes are"
* **Power Boost**: 0

### Steal the Project:
* **Tier**: 4
* **Input**: Pay Mana
* **Output**: Gain Steal
* **Flavor**: "Now there's no way they get a better score then me"
* **Power Boost**: 2

### Swap the Project:
* **Tier**: 4
* **Input**: Pay Steal
* **Output**: Gain Void
* **Flavor**: "I liked theirs more from the start"
* **Power Boost**: 1

### Chain Reaction:
* **Tier**: 4
* **Input**: Pay Void
* **Output**: Gain Ignite
* **Flavor**: "I don't know how it happened, my spell went haywire and just set theirs off!"
* **Power Boost**: 1

### Recursion:
* **Tier**: 5
* **Input**: Pay Life
* **Output**: Gain Perpetual
* **Flavor**: "Even a mage as powerful as myself has to pay a toll to perform something of this level"
* **Power Boost**: 2